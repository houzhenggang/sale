        ...
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case  'Q_EXCEED_NUM_LIMIT' : text = '上传文件数量超过限制，最多可一次性上传30张!' ;
					break;
					case  'F_EXCEED_SIZE' : text = '文件大小超过限制，单张图片最大5M!';
					break;
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var div = '<div class="diyButton"> \
						<a class="diyStart" href="javascript:void(0)">确定上传</a>   \
